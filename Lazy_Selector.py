...
class Player(Options):
    ...
    def __init__(self, win, shared_dict: SyncManager.dict):
        ...
        self._screen_height = (
            self._root.winfo_screenheight() - 104
            if self._root.winfo_screenheight() >= 900
            else self._root.winfo_screenheight() - 84
        )
        ...
        self.progressbar_style.configure(
            "custom.Horizontal.TScale", background=Player.BG
        )
        self.progressbar_style.configure(
            "TButton", foreground="gray97", focuscolor="gray97"
        )
        ...
        self.file_menu.add_command(label="Open Folder", command=self._manual_add)
        ...
        self.theme_menu.add_command(
            label="Light", command=lambda: self._update_color("gray97", "black")
        )
        self.theme_menu.add_separator()
        self.theme_menu.add_command(
            label="Dark", command=lambda: self._update_color("gray28", "gray97")
        )

        self.about_menu = Menu(
            self.menubar, tearoff=0, selectcolor=Player.FG, fg=Player.FG, bg=Player.BG
        )
        ...
        self.about_menu.add_checkbutton(
            label="Reset preferences", command=self._remove_pref
        )
        self.about_menu.add_separator()
        ...
        if (rem_battery < 41) and (rem_battery > 16):
            notification.notify(
                ...
                app_icon=(
                    f"{DATA_DIR}\\app.ico"
                    if os.path.exists(f"{DATA_DIR}\\app.ico")
                    else None
                ),
            )
    ...
    def send_event(self, funcname: str, *args, **kwargs):
        """
    ...
    def _resize_listbox(self):
        ...
        if self.listbox.size() > 35:
            self._root.geometry(
                f"318x{self._screen_height}+" + f"{self._root.winfo_x()}+{5}"
            )
            if not self.tab_num:
        ...
        if len(search_string) > 1:
            ...
            if self.tab_num:
                ...
                if not self.isOffline:
                    ...
                    if is_url(search_string):
                        ...
                        self._search_history.add(
                            search_string
                        )  # save only online searches
                        self.search_str = (
                            search_string  # avoid saving link as last search
                        )
                ...
                try:
                    ...
                    pass
                # ---------------------------------back button-------------
                self.back_toplaylist_btn = Button(
                    self.controls_frame,
                    image=self.lpo_image,
                    bg="gray28",
                    pady=0,
                    relief="flat",
                    width=66,
                ...
                self.listbox.delete(0, "end")
                self.collected = [
                    song for song in self._all_files if search_str in song.lower()
                ]
    ...
    def _file_details(self):
        ...
        # thread
        self.threadpool.submit(self._file_metadata, filename, title)
    ...
    def _url_details(self):
        ...
        # get metadata
        self.threadpool.submit(self._url_metadata, url, title)
    ...
    def _on_refresh(self):
        ...
        self._all_files = [
            i for i in all_files if i.endswith(self._supported_extensions)
        ]
    ...
    def _addto_queue(self):
        ...
        try:
            ...
            if (not self.collected) and (
                i != self.index and i != self.index + 1
            ):  # if adding from main list
                item = self._all_files[i]
            ...
            # if adding from searched list
                ...
                (self.collected)
                and (i != self.collection_index and i != self.collection_index + 1)
                and self.collection_index != -1
            ):
                item = self.collected[i]
                try:
                    if i > self.collection_index:
    ...
    def _listbox_rightclick(self, event):
        ...
        if self.collected:
            ...
            # popup.add_separator()
            popup.add_command(
                label="Play Next in Playlist", command=self._addto_playlist
            )
        else:
    ...
    def _play_options(self, event):
        ...
        popup.add_checkbutton(
            label="Mute", variable=self.mute_variable, command=self.mute_mixer
        )
        popup.add_checkbutton(
            label="Loop One", variable=self.loopone_variable, command=self._onoff_repeat
        )
        try:
    ...
    def _download_audio(self):
        ...
        try:
            ...
            if sinfo:
                ...
                for_display = [
                    f"{i}.    {s.p}    {s.p_size}"
                    for i, s in enumerate(self.audio_streams, start=1)
                ]
                self.status_bar.configure(text="")
                if for_display:
                    quality = getquality(
                        self._root, title, "Audio", for_display, self.download_location
                    )
                    self.prepare_download(quality)
                else:
                    self.status_bar.configure(
                        text="Some data is missing: no audio streams..."
                    )
            else:
    ...
    def _download_video(self):
        ...
        try:
            ...
            if sinfo:
                ...
                for_display = [
                    f"{i}.    {s.p}    {s.p_size}"
                    for i, s in enumerate(self.video_streams, start=1)
                ]
                self.status_bar.configure(text="")

                if for_display:
                    quality = getquality(
                        self._root, title, "Video", for_display, self.download_location
                    )
                    self.prepare_download(quality)
                else:
                    self.status_bar.configure(
                        text="Some data is missing: no video streams..."
                    )
            else:
    ...
    def _init(self):
        ...
        self.playlist_btn = Button(
            self.main_frame,
            relief="flat",
            image=self.rpo_image,
            height=15,
            pady=0,
            padx=0,
            bg=Player.BG,
            command=self._listview,
        )
        self.playlist_btn.place(x=1, y=1)

        self._previous_btn = Button(
            self.main_frame,
            padx=0,
            bg=Player.BG,
            fg=Player.FG,
            command=self._play_prev_command,
        ...
        self._next_btn = Button(
            ...
            padx=0,
            bg=Player.BG,
            command=self._play_next_command,
            image=self.next_img,
            fg=Player.FG,
            relief="groove",
            width=40,
    ...
    def _listview(self):
        ...
        self.progressbar_style.configure(
            "custom.Horizontal.TScale", background="gray28"
        )
        self.progress_bar = Scale(
            self.controls_frame,
            command=self._slide,
        ...
        notebook_style.map(
            ...
            background=(("selected", "white smoke"),),
            font=(("selected", ("", 9, "bold")),),
        )
        ...
        self.listbox = Listbox(
            self.local_listview, selectmode="extended", **self.LISTBOX_OPTIONS
        )
        ...
        self.thread_updating_streams()
        if self._playing:
            self.duration_tip = ToolTip(
                self.progress_bar, f"Duration: {timedelta(seconds=self.duration)}"
            )
        ToolTip(self.searchbar, "Press 'enter' to search")
        if self.isStreaming:
            book.select(1)
    ...
    def _handle_stream_tab(self):
        ...
        self.notification = Label(
            self.streams_listview, bg="gray28", fg="white smoke", text="Fetching..."
        )
        ...
        if self.isOffline and self._title_link is None:
            ...
            self.reload_button = Button(
                ...
                padx=5,
                pady=3,
                command=self.thread_updating_streams,
            )
        ...
        # create listbox; scrollbar and try adding data
            ...
            self.stream_scrollbar = Scrollbar(
                self.streams_listview, command=self.listview.yview
            )
            ...
            if self._title_link is not None:
                ...
                self.status_bar = Label(
                    self.streams_listview, anchor="e", font=("Consolas", 8, "bold")
                )
    ...
    def _on_eop(self):
        ...
        self.repeat_folder = Button(
            self.done_frame,
            image=self.repeat_folder_img,
            command=self._repeat_ended,
            relief="flat",
        )
        self.repeat_folder.pack(padx=70, pady=0, side="left")
        self.add_folder = Button(
            self.done_frame,
            image=self.add_folder_img,
    ...
    def _update_theme(self):
        ...
        self.current_time_label["bg"], self.current_time_label["fg"] = (
            Player.BG,
            Player.FG,
        )
    ...
    def _refresher(self):
        ...
        if len(PASSED_FILES) > 0:
            ...
            PASSED_FILES.clear()
            t = (
                os.path.basename(self._songspath)
                if os.path.basename(self._songspath)
                else "Disk"
            )
            ...
            if (self._open_folder) or (not os.path.exists(file)):
                ...
                self._songspath = (
                    chosen_dir if chosen_dir != "." else ""
                )  # raise FileNotFoundError below
            ...
            if (file == self._songspath) and (self.index < len(self._all_files) - 1):
                ...
                try:
                    ...
                    FileNotFoundError
                ):  # if Cancel clicked in the dialog _songspath == ''
        ...
        if len(all_files) > 0:
            ...
            Player._CONFIG.update_inner(
                "folders", "last", self._songspath
            )  # save to config file
            self._all_files = [
                i for i in all_files if i.endswith(self._supported_extensions)
            ]
            ...
            t = (
                os.path.basename(self._songspath)
                if len(os.path.basename(self._songspath)) != 0
                else "Disk"
            )
            self._root.title(f"{t} - Lazy Selector")
        ...
        if not self._playing and (
            self.shuffle_mixer.state.value == 6 or self.shuffle_mixer.state.value == 0
        ):
            self.collection_index = -1
            self._uptime = 0
    ...
    def _mixer(self, load):
        ...
        try:
            ...
            if os.path.isfile(load):
                ...
                self._title_txt = self._convert(
                    os.path.splitext(os.path.basename(load))[0]
                )
    ...
    def on_eos(self):
        ...
        if (
            not self._song and not self.index
        ):  # if self.index is 0; play or shuffle btn clicked once
            ...
            try:
                ...
                showinfo(
                    self._root, "Feedback - Lazy Selector", "No folder was selected!"
                )
    ...
    def update_dur_tooltip(self, t):
        ...
        try:  # If it's not first time of calling this function
            ...
            pass
        self.duration_tip = ToolTip(
            self.progress_bar, f"Duration: {timedelta(seconds=t)}"
        )
    ...
    def _kill(self):
        ...
        try:
            ...
            is_paused, is_downloading = (
                self.shuffle_mixer.state.value == 4
            ), self.is_downloading()
            # if mixer paused
    ...
    def _about(self):
        ...
        if self.top is None:
            ...
            def mailto(event):
                ...
                open_tab(
                    f"mailto:?to=ernestondieki12@gmail.com&subject={subject}&body={body}",
                    new=2,
                )
            ...
            self.display_canvas.create_text(
                ...
                anchor="w",
                font=("New Times Roman", 10, "italic bold"),
            )
            self.display_canvas.create_image(
                180, canvas_xy()[1], image=self.portrait_image, anchor=side
            )
        ...
        if self._playing:
            ...
            if self.shuffle_mixer.state.value == 6 and not self.loopone_variable.get():
                ...
                if (
                    ...
                    and self.controls_frame is not None
                    and self.collection_index > -1
                ):
                    self.collection_index += 1
                    # self.listbox.selection_clear(0, "end")
            ...
            if os.path.isdir(arg):
                ...
                all_files.extend(
                    (
                        ...
                        if f.endswith(self._supported_extensions)
                    )
                )
